...
bool run_query(sqlite3* db, const std::string& sql, std::vector< user_record >& records)
{
  ...
  // Implement regex pattern to verify no aruguments were appended
  std::regex regex_pattern(R"(^SELECT[*,\w\s]+ FROM USERS( WHERE NAME='\w+')?$)", std::regex::icase);
  
  // Check supplied sql matches regex
  // If no match, display intrusion detection and return false
  if(std::regex_match(sql, regex_pattern) == false) {
	  std::cout << "Injection detected: "<< sql << std::endl;
	  return false;
  }
